...
class fork_join_scheduler {
 ...
 public:
  ...
  template <typename F>
  static void parfor(
      scheduler_t& scheduler, size_t start, size_t end, F&& f, 
      size_t granularity = 0, bool conservative = false) {
    if (end <= start) return;
    if (granularity == 0) {
      size_t done = get_granularity(start, end, f);
      granularity = std::max(
        done, (end - start) / static_cast<size_t>(128 * scheduler.num_threads));
      start += done;
    }
 ...
 private:
  ...
  static size_t get_granularity(size_t start, size_t end, F& f) {
    // =========================================================================
    // My addition: DO NOT ADD THE LINES! Somehow it will crash infra::sort()!.
    // if constexpr (true) {
    //   auto Nitem = end - start;
    //   if (Nitem == 0) return 0;
    //   if ( Nitem <= size_t(maxCore() * maxCore()) ) return 1; 
    // }
    // =========================================================================
    size_t done = 0;
    size_t sz = 1;
  ...
  template <typename F>
  static void parfor_(scheduler_t& scheduler, size_t start, 
                      size_t end, F& f, size_t granularity, bool conservative) {
