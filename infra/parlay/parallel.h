...
// Thread-count control used by Parlay's default scheduler. Keep this in
// parallel.h so vendored Parlay headers compile without the infra wrapper.
inline int default_num_workers() {
  auto n = std::thread::hardware_concurrency();
  return static_cast<int>(n == 0 ? 1 : n);
}

inline int userDefinedMaxCore = default_num_workers();

inline void activate(auto&& maxCore = 1000) {
  userDefinedMaxCore = std::max<int>(
    1, std::min<int>(default_num_workers(), maxCore));
}

inline int maxCore() { return std::max(1, userDefinedMaxCore); }
...
inline size_t num_workers() {
  return maxCore() == 1 ? 1 : internal::get_current_scheduler().num_workers();
}

inline size_t worker_id() {
  return maxCore() == 1 ? 0 : internal::get_current_scheduler().worker_id();
  // return internal::get_current_scheduler().worker_id();
}

template <typename F>
inline void parallel_for(size_t start, size_t end, F&& f, 
                         long granularity, bool conservative) {
  static_assert(std::is_invocable_v<F&, size_t>);
  if (start + 1 == end) {
    f(start);
  }
  else if (maxCore() == 1 or (end - start) <= static_cast<size_t>(granularity)) {
...
inline void par_do(Lf&& left, Rf&& right, bool conservative) {
  ...
  if (maxCore() == 1) {
    ...
    return fork_join_scheduler::pardo(
      internal::get_current_scheduler(), std::forward<Lf>(left), 
      std::forward<Rf>(right), conservative);  
  }
}
...
void execute_with_scheduler(unsigned int p, F&& f) {
  if (maxCore() == 1) std::invoke(std::forward<F>(f));
  else {
