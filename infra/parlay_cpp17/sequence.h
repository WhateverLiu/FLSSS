  ...
  value_type& at(size_t i) {
    if (i >= size()) {
      throw_exception_or_terminate<std::out_of_range>(
          "sequence access out of bounds: length = " + 
  ...
  template<typename Range,
    std::enable_if_t<!std::is_same_v<std::decay_t<Range>, value_type> &&
                      ...
                      std::is_constructible_v<
                        value_type, range_reference_type_t<Range>>, int> = 0>
...
#pragma warning(pop)
 ...
 private:
  ...
  // ===========================================================================
  /*
  template <char indicator = 'i', bool singleThread = false>
  auto subset (auto && sub) { return subset(*this, sub); }
  
  
  template <char indicator = 'i', bool swap = false, bool singleThread = false>
  void subget (auto && sub, auto && assignment) { subget(*this, sub, assignment); }
  
  
  template <typename Int = unsigned, bool singleThread = false>
  auto which (auto && bf) { return which(*this, bf); }
  */
