...
auto group_by_key_ordered(Range&& S, Comp&& less) {
  ...
  constexpr bool uses_default_ascending_order =
      std::is_same_v<std::decay_t<Comp>, std::less<>> ||
      std::is_same_v<std::decay_t<Comp>, std::less<K>>;
  constexpr bool uses_default_descending_order =
      std::is_same_v<std::decay_t<Comp>, std::greater<>> ||
      std::is_same_v<std::decay_t<Comp>, std::greater<K>>;
  if constexpr(std::is_integral_v<K> && std::is_unsigned_v<K> && sizeof(KV) <= 16 &&
               (uses_default_ascending_order || uses_default_descending_order)) {
  ...
  if constexpr(std::is_integral_v<K> && std::is_unsigned_v<K> && sizeof(KV) <= 16 &&
               uses_default_descending_order) {
    auto idx = block_delayed::filter(ids, [&] (size_t i) {
      return (i==0) || internal::get_key(sorted[i-1]) < internal::get_key(sorted[i]); });

    return internal::tabulate(idx.size(), [&] (size_t i) {
      size_t group_id = idx.size() - 1 - i;
      size_t start = idx[group_id];
      size_t end = ((group_id==idx.size()-1) ? n : idx[group_id+1]);
      return std::pair(std::move(internal::get_key(sorted[idx[group_id]])),
