...
template <typename Val, typename Core>
[[nodiscard]] FLSSSGenResult FLSSS_gen_run(
    ...
    Core&& core)
{
    ...
    if (nrow == 0 || ncol == 0 || nSolutionsNeeded == 0) {
        return dispatchSignedWidth(
            minSignedByteSizeIndexCount(nrow),
            []<typename Ind>() {
                return make_gen_result(vec<vec<Ind>>{});
            });
    }
    ...
    ensureMatrixValueWidth<Val>(maxColSum);

    return dispatchSignedWidth(
        minSignedByteSizeIndexCount(nrow),
        [&]<typename Ind>() {
            return core.template operator()<Ind>(
                X, nrow, ncol, len,
                targetSumLowerBound,
                targetSumUpperBound,
